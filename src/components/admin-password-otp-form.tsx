...
export function AdminPasswordOtpForm({ email }: { email: string }) {
  ...
  async function sendCode() {
    ...
    try {
      const response = await fetch("/api/admin/profile/password/send-code", {
        method: "POST",
      });
  ...
  async function updatePassword(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/admin/profile/password/update", {
        method: "POST",
